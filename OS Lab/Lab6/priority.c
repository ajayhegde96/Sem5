...
int main()
{
	...
	for(int i=0;i<n;i++)
	scanf("%d%d%d",&(f[i].ar),&(f[i].bt),&(f[i].pr));
	for(int i=0;i<n;i++)
	{
	...
	int counter=1;
	int completed=f[0].bt;
	f[0].ct=f[0].bt;
	f[0].tat=f[0].ct-f[0].ar;
	f[0].wt=f[0].tat-f[0].bt;
	f[0].vis=1;
	int min=max;
	int pos;
	...
	while(counter<n){
		...
		for(int i=0;i<n;i++)
		{
			if(f[i].pr<min&&f[i].vis!=1&&f[i].ar<=completed)
			{
				pos=i;
				min=f[i].pr;
		...
		f[pos].vis=1;
		f[pos].wt=completed;
		f[pos].ct=completed+f[pos].bt;
		f[pos].tat=f[pos].ct-f[pos].ar;
		avgt+=f[pos].tat;
		avgw+=f[pos].wt;
		completed+=f[pos].bt;
		counter++;
